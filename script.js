...
/*
  V6.1 MAP FIX
  The old version used a rectangular grid for markerPositions, which meant
  the corner buttons were not actually sitting on the circuit.

  V6.1 now reads the real SVG track line and samples the required number of
  points along that line. This keeps every corner button on the actual track
  shape and makes the layout responsive without touching the engineer,
  timing, notes, or audio systems.
*/
function fallbackMarkerPositions(count){
  const shape=FALLBACK_SHAPE;
  const pts=[];
  for(let i=0;i<count;i++){
    const p=(i+0.5)/count;
    const scaled=p*shape.length;
    const a=Math.floor(scaled)%shape.length;
    const b=(a+1)%shape.length;
    const f=scaled-Math.floor(scaled);
    pts.push([
      shape[a][0]+(shape[b][0]-shape[a][0])*f,
      shape[a][1]+(shape[b][1]-shape[a][1])*f
    ]);
  }
  return pts;
}

async function getActualTrackMarkerPositions(mapUrl,count){
  if(!mapUrl || !count) return null;
  ...
  try{
    ...
    if(viewBox.length!==4 || viewBox.some(Number.isNaN)) throw new Error('No SVG viewBox');

    const host=document.createElementNS('http://www.w3.org/2000/svg','svg');
    host.setAttribute('viewBox',viewBox.join(' '));
    host.style.cssText='position:absolute;width:1px;height:1px;left:-99999px;top:-99999px;visibility:hidden';
    document.body.appendChild(host);

    const candidates=[...sourceSvg.querySelectorAll('path,polyline,polygon')];
    let best=null;
    let bestLength=0;
    ...
    for(const node of candidates){
      ...
      let length=0;
      try{
        if(typeof clone.getTotalLength==='function') length=clone.getTotalLength();
      }catch(e){}
      if(length>bestLength){
        if(best) host.removeChild(best);
        best=clone;
        bestLength=length;
      }else{
    ...
    if(!best || !bestLength){
      host.remove();
      throw new Error('No measurable track path');
    }

    const [,vx,vy,vw,vh]=[0,...viewBox];
    const positions=[];
    for(let i=0;i<count;i++){
      // Keep markers distributed around the real circuit line.
      const fraction=(i+0.5)/count;
      const point=best.getPointAtLength(bestLength*fraction);
      positions.push([
        ((point.x-vx)/vw)*100,
        ((point.y-vy)/vh)*100
      ]);
    ...
    if(positions.some(p=>p.some(v=>!Number.isFinite(v)))) throw new Error('Invalid marker coordinates');
    return positions;
  }catch(error){
    console.warn('V6.1: Could not read track SVG for marker placement.',error);
    return null;
  }
}

function buildMapMarkers(positions,t){
  return positions.map((p,i)=>`
    <button class="map-corner ${i===state.selectedCorner?'selected':''}"
      style="left:${p[0]}%;top:${p[1]}%"
      data-index="${i}"
      title="${escapeHtml(t.turns[i])}"
      aria-label="${escapeHtml(t.turns[i])}">
      ${i+1}
    </button>`).join('');
}

function attachMapButtons(){
  document.querySelectorAll('.map-corner').forEach(b=>{
    b.addEventListener('click',()=>selectCorner(Number(b.dataset.index)));
  });
}
...
async function renderMap(){
  ...
  // Start with a track-following fallback instead of the old rectangle.
  const fallbackPositions=fallbackMarkerPositions(t.turns.length);
  const markers=buildMapMarkers(fallbackPositions,t);

  const map=t.mapUrl
    ? `<img src="${t.mapUrl}" alt="${escapeHtml(t.name)} circuit layout" loading="eager"
  ...
  // Once the actual SVG is readable, replace the fallback positions with
  // points sampled directly from the real circuit geometry.
  if(t.mapUrl){
    const actual=await getActualTrackMarkerPositions(t.mapUrl,t.turns.length);
    if(actual && getTrack().id===t.id){
      const overlay=$('mapWrap').querySelector('.map-overlay');
      if(overlay){
        overlay.innerHTML=buildMapMarkers(actual,t);
        attachMapButtons();
      }
    }
  }
